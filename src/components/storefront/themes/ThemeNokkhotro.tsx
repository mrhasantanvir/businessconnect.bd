    ...
    <div className="min-h-screen bg-[#F9FAFB] text-[#0F172A] selection:bg-indigo-100 font-sans">
            ...
            <div className="flex items-center gap-12">
               ...
               <nav className="hidden lg:flex items-center gap-8">
                  {["Collection", "New Arrivals", "Premium Pick", "Story"].map(item => (
                    <a key={item} href="#" className="text-[10px] font-black uppercase tracking-widest hover:text-indigo-600 transition-colors">
                       {item}
                    </a>
                  ))}
               </nav>
            ...
            <div className="flex items-center gap-6">
               ...
               <div className="flex items-center gap-4">
                        ...
                        <div className="absolute top-full right-0 mt-2 bg-white border border-slate-100 rounded-2xl shadow-2xl p-2 z-[200] min-w-[100px] animate-in fade-in slide-in-from-top-2">
                           {["BDT", "USD", "EUR", "GBP"].map(code => (
                              <button 
                                 ...
                                 className={cn(
                                   "w-full text-left px-4 py-2 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-50 transition-all",
                  ...
                  <div className="relative p-2 hover:bg-slate-100 rounded-full transition-all group cursor-pointer" onClick={() => window.dispatchEvent(new CustomEvent("toggleCart"))}>
                     ...
                     <span className="absolute top-0 right-0 w-4 h-4 bg-indigo-600 text-white text-[8px] font-black flex items-center justify-center rounded-full shadow-lg">{cartCount}</span>
                     ...
                     <div className="absolute top-full right-0 mt-4 w-64 bg-white border border-slate-100 rounded-3xl shadow-2xl p-6 opacity-0 translate-y-4 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 transition-all z-50">
                        ...
                        <button className="w-full py-3 bg-indigo-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest">Checkout Now</button>
                     </div>
                  </div>
               </div>
            </div>
                     ...
                     <div className="grid grid-cols-4 gap-2">
                        {["BDT", "USD", "EUR", "GBP"].map(code => (
                           <button 
                              ...
                              className={cn(
                                "py-3 text-[10px] font-black uppercase rounded-xl transition-all border",
      ...
      <section className="relative h-[100vh] overflow-hidden flex items-center">
         ...
         <div className="max-w-[1400px] mx-auto w-full px-8 relative z-10 space-y-10 animate-in fade-in slide-in-from-left-20 duration-1000">
            ...
            <button className="px-12 py-6 bg-slate-900 text-white rounded-full text-[10px] font-black uppercase tracking-[0.3em] flex items-center gap-6 hover:bg-indigo-600 transition-all shadow-2xl hover:scale-105">
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8">
            {["Grocery", "Fashion", "Electronics", "Beauty", "Home", "Gifts"].map((cat, i) => (
              <div key={i} className="group space-y-4 text-center cursor-pointer">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover:text-slate-900 transition-colors">{cat}</span>
              </div>
            ))}
         </div>
      </section>
              ...
              >
                 <div className="aspect-[3/4] bg-white rounded-[40px] overflow-hidden relative shadow-sm hover:shadow-2xl transition-all duration-700">
                    ...
                    <div className="absolute top-6 left-6 flex flex-col gap-2">
                       <span className="px-3 py-1.5 bg-white text-[#0F172A] text-[8px] font-black uppercase tracking-widest rounded-full shadow-xl">AI Choice</span>
                    </div>
                    ...
                    <div className="absolute bottom-6 left-6 right-6 translate-y-10 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                       <button 
                          ...
                          className="w-full py-4 bg-slate-900 text-white rounded-2xl text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-indigo-600 shadow-2xl"
                       >
                 ...
                 <div className="space-y-2 px-4">
                    <div className="flex items-center justify-between">
                       ...
                       <span className="text-sm font-black text-indigo-600">{formatPrice(product.price)}</span>
                    </div>
            ...
            <div className="flex gap-4 md:gap-12 relative z-10">
               {["04", "12", "45", "08"].map((time, i) => (
                 <div key={i} className="flex flex-col items-center">
                    <div className="text-4xl lg:text-7xl font-black tracking-tighter">{time}</div>
                    <span className="text-[10px] font-black uppercase tracking-widest opacity-60">{["Days", "Hours", "Mins", "Secs"][i]}</span>
                 </div>
               ))}
            </div>

            <button className="px-12 py-5 bg-white text-indigo-600 rounded-full text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all relative z-10 shadow-2xl">
               Claim Discount Now
            </button>
         </div>
      </section>
      ...
      <footer className="bg-slate-900 py-32 px-8 text-white">
         <div className="max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-20">
            <div className="col-span-1 lg:col-span-2 space-y-12">
               ...
               <div className="max-w-md space-y-6">
                  ...
                  <div className="flex border-b-2 border-slate-700 pb-4">
                     <input placeholder="ENTER EMAIL ADDRESS" className="bg-transparent border-none outline-none text-[10px] font-black w-full" />
                     <button className="hover:text-indigo-400 transition-all"><ArrowRight className="w-6 h-6" /></button>
                  </div>
               </div>
            </div>
            <div className="space-y-8">
               <h5 className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-400">Information</h5>
            ...
            <div className="space-y-8 text-right">
               <h5 className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-400">Location</h5>
      ...
      <div className="fixed bottom-10 right-10 z-[200]">
         <button className="w-16 h-16 bg-indigo-600 text-white rounded-full flex items-center justify-center shadow-2xl shadow-indigo-600/40 hover:scale-110 transition-all relative group">
            ...
            <div className="absolute right-full mr-4 px-6 py-3 bg-white text-slate-900 text-slate-900 border border-slate-100 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0 transition-all">
               Ask AI Assistant
            </div>
         </button>
      </div>
