  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] pb-20 print:bg-white print:pb-0">
      ...
      <header className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-8 lg:px-20 print:hidden">
         <button onClick={() => router.push(`/s/${slug}`)} className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-indigo-600 transition-colors">
            <ArrowLeft className="w-4 h-4" /> Back to Store
         </button>
         <div className="flex items-center gap-4">
            <button onClick={handlePrint} className="flex items-center gap-2 px-6 py-2.5 bg-slate-900 text-white rounded-full text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all">
      ...
      <main className="max-w-[1000px] mx-auto px-8 lg:px-20 py-16 space-y-12">
         ...
         <div className="bg-white border border-slate-100 rounded-[48px] p-12 shadow-sm space-y-10 print:hidden">
            <h3 className="text-xs font-black uppercase tracking-[0.3em] text-slate-400 text-center">Tracking Status</h3>
               ...
               {steps.map((step, i) => (
                  <div key={i} className="relative z-10 flex flex-col items-center gap-4 group">
                     ...
                     <div className="text-center">
                        <p className={cn("text-[9px] font-black uppercase tracking-widest", step.status !== "pending" ? "text-slate-900" : "text-slate-300")}>{step.label}</p>
         ...
         <div id="invoice" className="bg-white border border-slate-200 rounded-[48px] p-16 shadow-2xl print:border-none print:shadow-none print:p-0 print:rounded-none">
            ...
            <div className="flex justify-between items-start border-b border-slate-100 pb-12">
               ...
               <div className="text-right space-y-4">
                  <div className="px-6 py-2 bg-emerald-50 text-emerald-600 rounded-full text-[9px] font-black uppercase tracking-widest inline-block border border-emerald-100">
            ...
            <div className="grid grid-cols-2 gap-12 py-12 border-b border-slate-100">
               ...
               <div className="space-y-4">
                  <div className="flex items-center gap-3">
                     ...
                     <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Logistics Partner</h4>
                  </div>
                  <p className="text-xs font-bold text-slate-600 uppercase">
                     {order.preferredCourier || "Standard Local Shipping"}
                  </p>
                  {order.trackingCode && (
                     <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest bg-indigo-50 px-3 py-1 rounded-full inline-block">
            ...
            <div className="py-12 space-y-8">
               <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Purchased Items</h4>
               <table className="w-full">
                  <thead>
                     <tr className="text-left border-b border-slate-50 pb-4">
                        <th className="text-[9px] font-black uppercase tracking-widest text-slate-400 pb-4">Product Description</th>
                        <th className="text-[9px] font-black uppercase tracking-widest text-slate-400 pb-4 text-center">Qty</th>
                        <th className="text-[9px] font-black uppercase tracking-widest text-slate-400 pb-4 text-right">Unit Price</th>
                        <th className="text-[9px] font-black uppercase tracking-widest text-slate-400 pb-4 text-right">Total</th>
                     </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                     {order.items.map((item: any, i: number) => (
                        <tr key={i} className="group">
                           ...
                           <td className="py-6 text-right text-[11px] font-black text-slate-900">৳{item.price.toLocaleString()}</td>
                           <td className="py-6 text-right text-[11px] font-black text-indigo-600">৳{(item.price * item.quantity).toLocaleString()}</td>
                        </tr>
                     ))}
                  </tbody>
               </table>
            ...
            <div className="border-t border-slate-100 pt-12 flex justify-end">
               <div className="w-full max-w-[300px] space-y-4">
                  ...
                  <div className="pt-4 flex justify-between items-end border-t border-slate-100">
                     <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-900">Total Paid</span>
                     <span className="text-4xl font-black text-indigo-600 tracking-tighter leading-none">৳{order.total.toLocaleString()}</span>
                  </div>
               </div>
            </div>
               ...
               <div className="mt-12 p-8 bg-red-50 border-2 border-dashed border-red-200 rounded-[32px] space-y-6">
                  ...
                  >
                     ...
                     <input 
                        ...
                        required
                     />
                     <button className="w-full py-4 bg-red-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-red-700 transition-all shadow-xl shadow-red-200">
