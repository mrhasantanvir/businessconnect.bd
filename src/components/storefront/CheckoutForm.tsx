    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-8 text-center space-y-8 animate-in fade-in zoom-in-95 duration-700">
         ...
         <div className="space-y-4">
            <h1 className="text-2xl font-bold uppercase tracking-tight">Order <span className="text-green-600">Confirmed</span></h1>
         ...
         <button onClick={() => router.push(`/s/${slug}`)} className="px-12 py-5 bg-slate-900 text-white rounded-full text-[10px] font-bold uppercase tracking-widest hover:scale-105 transition-all">
            Return to Store
         </button>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] text-[#0F172A] font-sans selection:bg-indigo-100">
      <header className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-8 lg:px-20">
         <button onClick={() => router.back()} className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-indigo-600 transition-colors">
            <ArrowLeft className="w-4 h-4" /> Back to Shopping
         </button>
         <h1 className="text-xl font-bold tracking-tight uppercase text-indigo-600">Checkout Portal</h1>
         <div className="flex items-center gap-2 text-emerald-600 font-bold text-[9px] uppercase tracking-widest bg-emerald-50 px-4 py-2 rounded-full border border-emerald-100">
      ...
      <main className="max-w-[1200px] mx-auto px-8 lg:px-20 py-20 grid grid-cols-1 lg:grid-cols-2 gap-20">
         <div className="space-y-12">
            <div>
               <h2 className="text-2xl font-bold uppercase tracking-tight">Shipping <span className="text-indigo-600">Destination</span></h2>
            ...
            <form id="checkout-form" onSubmit={handlePlaceOrder} className="space-y-8">
               <div className="space-y-6">
                  <div className="group space-y-2">
                     <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-4 group-focus-within:text-indigo-600 transition-colors">Recipient Name</label>
                  ...
                  <div className="group space-y-2">
                     <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-4 group-focus-within:text-indigo-600 transition-colors">Active Phone Number</label>
                     <div className="flex items-center gap-4 bg-white border border-slate-100 p-5 rounded-[24px] focus-within:border-indigo-600 focus-within:shadow-2xl focus-within:shadow-indigo-600/5 transition-all">
                  ...
                  <div className="group space-y-2">
                     <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-4 group-focus-within:text-indigo-600 transition-colors">Email Address (Optional)</label>
                     <div className="flex items-center gap-4 bg-white border border-slate-100 p-5 rounded-[24px] focus-within:border-indigo-600 focus-within:shadow-2xl focus-within:shadow-indigo-600/5 transition-all">
                  ...
                  <div className="group space-y-2">
                     <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-4 group-focus-within:text-indigo-600 transition-colors">Detailed Address</label>
               ...
               <div className="pt-8 space-y-6">
                  <h3 className="text-xl font-bold uppercase tracking-tight">Shipping <span className="text-indigo-600">Method</span></h3>
                     ...
                     {[
                        ...
                        >
                           <div className="flex items-center gap-6">
                              ...
                              <div>
                                 <h4 className="text-[10px] font-bold uppercase tracking-widest">{method.name}</h4>
                                 <p className="text-[9px] font-bold text-slate-400 uppercase mt-1">Delivery: {method.time}</p>
                              </div>
                           </div>
                           <div className="text-right">
                              <span className="text-xs font-bold uppercase">{method.price === 0 ? "Free" : formatPrice(method.price)}</span>
                           </div>
                        </div>
                     ))}
                  </div>
               ...
               <div className="pt-8 space-y-6">
                  <h3 className="text-xl font-bold uppercase tracking-tight">Payment <span className="text-indigo-600">Gateway</span></h3>
                        ...
                        >
                           <div className="flex items-center gap-4">
                              ...
                              <div className="text-left">
                                 <span className="text-[10px] font-bold uppercase tracking-widest block">{config.provider}</span>
            ...
            <div className="bg-white border border-slate-100 rounded-[48px] p-10 shadow-sm space-y-8">
                     ...
                     {cart.map((item) => (
                        <div key={item.id} className="flex items-center justify-between gap-4 group">
                           <div className="flex items-center gap-4">
                              ...
                              <div>
                                 <h4 className="text-[11px] font-bold uppercase text-slate-900 leading-none">{item.name}</h4>
               ...
               <div className="pt-8 border-t border-slate-100 space-y-4">
                  ...
                  <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                     ...
                     <span className="text-emerald-600 font-bold">{shippingMethod.price === 0 ? "FREE" : formatPrice(shippingMethod.price)}</span>
                  </div>
                  <div className="pt-4 flex justify-between items-end">
                     <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-900">Total To Pay</span>
                     <span className="text-2xl font-bold text-indigo-600 tracking-tight leading-none">{formatPrice(total + shippingMethod.price)}</span>
                  </div>
               </div>
