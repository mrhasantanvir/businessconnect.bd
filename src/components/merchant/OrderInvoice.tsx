      ...
      <div className="flex justify-between items-start border-b-4 border-indigo-600 pb-12 mb-12">
         ...
         <div className="text-right space-y-2">
            ...
            <div className="relative z-10">
               ...
               <p className="text-sm font-black text-slate-900">{new Date(order.createdAt).toLocaleDateString()}</p>
            </div>
         </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-20 mb-16">
         <div className="space-y-4">
            ...
            <div className="space-y-2">
               ...
               <p className="text-xs font-black text-slate-900">{order.customerPhone}</p>
            </div>
         </div>
         <div className="space-y-4 p-8 bg-slate-50 rounded-[32px] border border-slate-100">
            <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Payment Status</h4>
            <div className="flex items-center gap-3">
               <div className={cn(
               ...
               <span className="text-xs font-black text-slate-900 uppercase tracking-tighter">via Cash on Delivery</span>
            </div>
            <div className="flex items-center gap-2 pt-2 text-[8px] font-black text-slate-400 uppercase tracking-widest">
         ...
         <table className="w-full text-left border-collapse">
            <thead>
               <tr className="border-b-2 border-slate-100">
                  <th className="py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Description</th>
                  <th className="py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-center">Qty</th>
                  <th className="py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-right">Unit Price</th>
                  <th className="py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-right">Total</th>
               </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
               {order.items.map((item: any) => (
                 <tr key={item.id} className="group">
                    <td className="py-8">
                       <h6 className="text-sm font-black text-slate-900 uppercase tracking-tight">{item.product.name}</h6>
                       <p className="text-[9px] font-bold text-slate-400 uppercase mt-1">SKU: {item.product.id.slice(0, 8)}</p>
                    </td>
                    <td className="py-8 text-center text-xs font-black text-slate-900">{item.quantity}</td>
                    <td className="py-8 text-right text-xs font-bold text-slate-500">৳{item.price.toLocaleString()}</td>
                    <td className="py-8 text-right text-sm font-black text-slate-900">৳{(item.price * item.quantity).toLocaleString()}</td>
                 </tr>
               ))}
            </tbody>
         </table>
      ...
      <div className="grid grid-cols-2 gap-12 pt-8 border-t-2 border-slate-100">
            ...
            <div className="flex gap-4 pt-4">
               {store.printSettings?.showCustomerSignature && (
                  <div className="flex-1 border-t border-slate-900 pt-2">
                     <p className="text-[9px] font-black text-center uppercase tracking-widest text-slate-400">Customer Signature</p>
                  </div>
               )}
               {store.printSettings?.showMerchantSignature && (
                  <div className="flex-1 border-t border-slate-900 pt-2">
                     <p className="text-[9px] font-black text-center uppercase tracking-widest text-slate-400">Authorized Signature</p>
                  </div>
               )}
            </div>
         </div>
         <div className="space-y-6">
            ...
            <div className="pt-6 border-t border-slate-200 flex justify-between items-end">
               <span className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-600">Total Amount</span>
               <span className="text-4xl font-black text-slate-900 tracking-tighter leading-none">৳{grandTotal.toLocaleString()}</span>
            </div>
         </div>
      </div>
