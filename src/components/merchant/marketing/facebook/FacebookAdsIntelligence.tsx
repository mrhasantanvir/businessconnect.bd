      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-gray-100  pb-8">
        <div>
           <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-2xl md:text-2xl font-bold text-[#0F172A]  uppercase tracking-tight">
        ...
        <div className="flex items-center gap-3">
           <button className="flex items-center gap-2 px-6 py-2.5 bg-white  border border-gray-100  rounded-2xl text-[10px] font-bold uppercase hover:bg-gray-50 transition-all shadow-sm">
        ...
        <div className="space-y-8">
           ...
           <div className="bg-white  border border-gray-100  rounded-[40px] p-8 md:p-12 shadow-sm h-[400px] flex items-center justify-center relative overflow-hidden">
              ...
              <div className="text-center space-y-4 relative z-10">
                 ...
                 <h3 className="text-xl font-bold text-[#0F172A] ">ROAS vs Ad Spend Visualization</h3>
           ...
           {[1, 2, 3].map(i => (
             <div key={i} className="bg-white  border border-gray-100  rounded-3xl overflow-hidden group hover:border-indigo-500/30 transition-all shadow-sm">
                <div className="aspect-[4/5] bg-gray-50  flex items-center justify-center relative overflow-hidden">
                   ...
                   <div className="absolute top-4 right-4 px-3 py-1 bg-green-500 text-white text-[8px] font-bold uppercase rounded-full shadow-lg">Active Ad</div>
                </div>
                <div className="p-6 space-y-4">
                   <div className="flex items-center gap-3">
                      <div className="w-8 h-8 bg-gray-100  rounded-lg"></div>
                      <div>
                         <h4 className="text-xs font-bold">Competitor Brand</h4>
                   ...
                   <button className="w-full py-3 bg-gray-50  border border-gray-100  rounded-xl text-[9px] font-bold uppercase tracking-widest flex items-center justify-center gap-2 group-hover:bg-indigo-600 group-hover:text-white transition-all">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
           <div className="bg-white  border border-gray-100  rounded-[40px] p-8 md:p-12 space-y-8">
              <div className="space-y-4">
                 <label className="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Product Selection</label>
                 <select 
              ...
              <div className="space-y-4">
                 <label className="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Ad Objective</label>
                 <div className="grid grid-cols-2 gap-3">
                    <button 
                      onClick={() => setObjective("Sales Conversion")}
                      className={`p-4 rounded-2xl text-[10px] font-bold uppercase transition-all ${objective === 'Sales Conversion' ? 'bg-indigo-50 border border-indigo-100 text-indigo-600' : 'bg-white border border-gray-100 text-gray-400'}`}
                    >
                      Sales Conversion
                    </button>
                    <button 
                      onClick={() => setObjective("Brand Awareness")}
                      className={`p-4 rounded-2xl text-[10px] font-bold uppercase transition-all ${objective === 'Brand Awareness' ? 'bg-indigo-50 border border-indigo-100 text-indigo-600' : 'bg-white border border-gray-100 text-gray-400'}`}
                    >
                      Brand Awareness
                    </button>
                 </div>
              ...
              <button 
                ...
                className="w-full py-5 bg-[#1877F2] text-white rounded-2xl text-xs font-bold uppercase shadow-lg shadow-blue-200 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
              >
                 {isGenerating ? (
           ...
           <div className="bg-indigo-600 text-white rounded-[40px] p-8 md:p-12 flex flex-col justify-between shadow-2xl shadow-indigo-200 min-h-[400px]">
              <div className="space-y-6">
                 ...
                 {generatedCopy ? (
                   ...
                   <>
                     <h3 className="text-lg md:text-lg font-bold tracking-tight leading-tight">
              ...
              {generatedCopy && (
                <div className="pt-8 border-t border-white/10 mt-12 flex items-center justify-between">
                   <div className="text-[10px] font-bold uppercase tracking-widest">Optimized for ROAS</div>
                   <button 
                     onClick={() => {
                        navigator.clipboard.writeText(generatedCopy);
                        toast.success("Copied to clipboard!");
                     }}
                     className="px-6 py-2.5 bg-white text-indigo-600 rounded-xl text-[10px] font-bold uppercase hover:bg-gray-100 transition-all"
                   >
                     Copy to Ads
                   </button>
                </div>
  ...
  return (
    <div className="bg-white  border border-gray-100  rounded-3xl p-5 md:p-6 shadow-sm group hover:border-indigo-500/30 transition-all">
       <div className="flex items-center justify-between mb-4">
          ...
          <div className={`text-[8px] md:text-[9px] font-bold px-2 py-0.5 rounded-full border ${
            isPositive ? 'bg-green-50 text-green-600 border-green-100' : 'bg-red-50 text-red-600 border-red-100'
          }`}>
             {trend}
          </div>
       </div>
       <div className="text-lg md:text-xl font-bold text-[#0F172A]  tracking-tight">{value}</div>
       <h4 className="text-[8px] md:text-[9px] font-bold uppercase text-gray-400 tracking-widest mt-1">{title}</h4>
    </div>
  );
}
