    ...
    <div className="w-[800px] mx-auto bg-white p-10 border-2 border-slate-200 rounded-[32px] font-outfit shadow-2xl print:shadow-none print:border-none print:p-0">
      ...
      <div className="flex justify-between items-start border-b-4 border-slate-900 pb-8 mb-8">
        ...
        <div className="text-right">
          <div className="bg-slate-900 text-white px-6 py-2 rounded-full font-black text-sm uppercase tracking-widest mb-2">
      ...
      <div className="grid grid-cols-2 gap-12 mb-12">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
            <p className="font-black text-xl text-slate-900 mb-2">{order.customerName}</p>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-indigo-50/50 p-6 rounded-3xl border border-indigo-100">
            <p className="font-black text-xl text-indigo-900 mb-2">{order.merchantStore?.name || "Main Hub"}</p>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-slate-900 text-white">
              <th className="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest rounded-l-2xl">Item SKU</th>
              <th className="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest">Description</th>
              <th className="px-6 py-4 text-center text-[10px] font-black uppercase tracking-widest rounded-r-2xl">Qty</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {order.items.map((item: any) => (
              <tr key={item.id}>
                ...
                <td className="px-6 py-4 font-black text-sm text-slate-900">{item.product?.name || item.name}</td>
      ...
      <div className="grid grid-cols-3 gap-8 items-end">
        <div className="col-span-2 space-y-6">
          <div className="p-6 bg-amber-50 border-2 border-dashed border-amber-200 rounded-3xl">
            <h4 className="text-[10px] font-black text-amber-700 uppercase mb-2">Instructions for Logistics</h4>
          ...
          <div className="flex items-center gap-6">
            <div className="flex-1 p-6 bg-slate-50 rounded-3xl border border-slate-100">
              <p className="text-[9px] font-black text-slate-400 uppercase mb-2">Original Order ID</p>
              <p className="font-mono font-black text-slate-900">#{order.id.slice(-12).toUpperCase()}</p>
            </div>
            <div className="flex-1 p-6 bg-slate-50 rounded-3xl border border-slate-100">
        ...
        <div className="flex flex-col items-center gap-4">
           ...
           />
           <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-900">SCAN TO RECEIVE</p>
        </div>
      </div>
      ...
      <div className="mt-12 pt-8 border-t border-slate-100 flex justify-between items-center opacity-50">
         <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest">Verified Merchant Return</span>
         </div>
