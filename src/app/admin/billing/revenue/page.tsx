  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight uppercase">Revenue <span className="text-indigo-600">Intelligence</span></h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         ...
         <div className="p-8 bg-white border border-slate-100 rounded-[32px] shadow-sm relative overflow-hidden group">
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Successful Payments</p>
            <p className="text-2xl font-bold text-slate-900">{recentTransactions.length}+</p>
            <div className="mt-4 flex items-center gap-1.5">
               <span className="text-[10px] font-bold bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full">LIVE</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="bg-white border border-slate-100 rounded-[32px] p-8 shadow-sm lg:col-span-1">
            <h3 className="text-lg font-bold text-slate-900 uppercase tracking-tight mb-8">Revenue <span className="text-indigo-600">Sources</span></h3>
               ...
               {typeBreakdown.map((item: any) => (
                  <div key={item.type} className="group">
                     ...
                     <div className="flex justify-between mt-1.5">
                        ...
                        <span className="text-[9px] font-bold text-slate-500">{((item._sum.amount / (totalRevenue || 1)) * 100).toFixed(1)}%</span>
                     </div>
                  </div>
               ))}
            </div>
         ...
         <div className="bg-white border border-slate-100 rounded-[32px] overflow-hidden shadow-sm lg:col-span-2">
            <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
               <div>
                  <h3 className="text-lg font-bold text-slate-900 uppercase tracking-tight">Global <span className="text-indigo-600">Ledger</span></h3>
                     ...
                     {recentTransactions.map((tx: any) => (
                        <tr key={tx.id} className="hover:bg-slate-50/30 transition-colors group">
                           <td className="px-8 py-6">
                              <div className="font-bold text-slate-900 tracking-tight">{tx.merchantStore?.name || "N/A"}</div>
                              <div className="text-[10px] text-slate-400 font-bold uppercase">{tx.user?.name || "Unknown User"}</div>
                           </td>
                           <td className="px-8 py-6">
                              <div className="flex flex-col gap-1">
                                 <span className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest">{tx.type.replace('_', ' ')}</span>
                           ...
                           <td className="px-8 py-6 text-right">
                              <div className="text-xs font-bold text-slate-900 uppercase tracking-tight">{format(tx.createdAt, "dd MMM, yy")}</div>
            ...
            <div className="p-6 bg-slate-50/50 text-center border-t border-slate-100">
               <button className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest hover:underline">Load More Financial Data</button>
            </div>
         </div>
      </div>
         ...
         {methodBreakdown.map((method: any) => (
            <div key={method.paymentMethod} className="bg-white border border-slate-100 rounded-[32px] p-8 shadow-sm relative overflow-hidden group">
               ...
               <div className="flex items-center justify-between mb-4">
                  <h4 className={`text-xs font-bold uppercase tracking-widest ${method.paymentMethod === 'BKASH' ? 'text-[#e2136e]' : 'text-[#f7941d]'}`}>{method.paymentMethod} Gateway</h4>
