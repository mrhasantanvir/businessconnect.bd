  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
         <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-5">
               <div className="w-20 h-20 bg-indigo-600 rounded-[32px] flex items-center justify-center text-white shadow-xl shadow-indigo-200">
                  <span className="text-3xl font-black">{customer.name?.[0].toUpperCase() || "C"}</span>
               </div>
               <div>
                  <div className="flex items-center gap-3">
                     <h1 className="text-3xl font-black text-[#0F172A]  tracking-tight">{customer.name || "Guest Customer"}</h1>
                     <div className="px-4 py-1.5 bg-[#BEF264]/10 text-green-700  border border-[#BEF264]/30 rounded-full text-[10px] font-black uppercase tracking-widest">
                        {customer.tags || "NEW"}
                     </div>
                  </div>
         ...
         <div className="flex items-center gap-3">
            <button className="flex items-center gap-2 px-6 py-4 bg-white  border border-gray-100  rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-50 transition-all">
               <MessageSquare className="w-4 h-4" /> Message
            </button>
            <button className="flex items-center gap-2 px-6 py-4 bg-white text-slate-900 text-slate-900 text-slate-900 border border-slate-100 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-gray-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white  border border-[#E5E7EB]  rounded-[40px] p-8 shadow-sm">
                   <div className="flex items-center gap-3 mb-6">
                      ...
                      <span className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Lifetime Value</span>
                   </div>
                   <h2 className="text-3xl font-black text-[#0F172A] ">৳{customer.totalSpend.toLocaleString()}</h2>
                ...
                <div className="bg-white  border border-[#E5E7EB]  rounded-[40px] p-8 shadow-sm">
                   <div className="flex items-center gap-3 mb-6">
                      ...
                      <span className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Loyalty Points</span>
                   </div>
                   <h2 className="text-3xl font-black text-[#0F172A] ">{customer.loyaltyPoints}</h2>
                ...
                <div className="bg-white  border border-[#E5E7EB]  rounded-[40px] p-8 shadow-sm">
                   <div className="flex items-center gap-3 mb-6">
                      ...
                      <span className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Order Frequency</span>
                   </div>
                   <h2 className="text-3xl font-black text-[#0F172A] ">{customer.orderCount}</h2>
                        ...
                        {customer.orders.map((order) => (
                           <tr key={order.id} className="hover:bg-gray-50  transition-all group">
                              <td className="px-8 py-6">
                                 <Link href={`/orders/${order.id}`} className="font-black text-indigo-600 hover:text-indigo-700 transition-colors flex items-center gap-2">
         ...
         <div className="space-y-8">
            ...
            <div className="bg-[#F8F9FA]  border border-[#E5E7EB]  rounded-[48px] p-8 space-y-8 shadow-sm">
               <h3 className="text-xs font-black uppercase text-indigo-500 tracking-[0.2em] mb-8">System Intelligence</h3>
               ...
               <div className="space-y-6">
                  <div className="flex items-start gap-5">
                     ...
                     <div>
                        <h4 className="text-[10px] font-black uppercase text-gray-400 mb-1">Customer Since</h4>
                        <p className="text-xs font-black text-[#0F172A] ">{new Date(customer.createdAt).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</p>
                     </div>
                  </div>
                  ...
                  <div className="flex items-start gap-5">
                     ...
                     <div>
                        <h4 className="text-[10px] font-black uppercase text-gray-400 mb-1">Primary Zone</h4>
                        <p className="text-xs font-black text-[#0F172A] ">Dhaka, Bangladesh</p>
                     </div>
                  </div>
                  ...
                  <div className="flex items-start gap-5">
                     ...
                     <div>
                        <h4 className="text-[10px] font-black uppercase text-gray-400 mb-1">Trust Score</h4>
                        <div className="flex items-center gap-1 mt-1">
                           {[1,2,3,4,5].map(s => <Star key={s} className="w-3 h-3 text-orange-400 fill-orange-400" />)}
                           <span className="text-[10px] font-black ml-2 text-indigo-500">Verified</span>
                        </div>
                     </div>
                  </div>
               </div>
               ...
               <div className="pt-8 border-t border-gray-100 ">
                  ...
                  <div className="flex flex-wrap gap-2">
                     {customer.tags.split(",").map(tag => (
                       <span key={tag} className="px-4 py-2 bg-white  border border-gray-100  rounded-xl text-[9px] font-black uppercase tracking-tight text-indigo-600 shadow-sm">
                          {tag.trim()}
                       </span>
                     ))}
                  </div>
                  ...
                  <div className="mt-8 p-6 bg-white  border border-indigo-50  rounded-[32px]">
                     <h5 className="text-[10px] font-black mb-2 flex items-center gap-2">
            ...
            <div className="bg-white text-slate-900 text-slate-900 text-slate-900 border border-slate-100 text-white rounded-[48px] p-10 space-y-6 relative overflow-hidden group">
                ...
                <h3 className="text-xl font-black tracking-tight leading-none">Internal Notes</h3>
