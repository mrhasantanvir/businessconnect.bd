    ...
    <div className="max-w-7xl mx-auto space-y-10 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
         ...
         <div className="flex items-center gap-4 bg-white p-2 rounded-3xl shadow-sm border border-slate-100">
            <div className="px-6 py-3 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl">
               All Reviews ({reviews.length})
            </div>
            <div className="px-6 py-3 text-slate-400 text-[10px] font-black uppercase tracking-widest hover:text-indigo-600 transition-colors">
               Pending Reply
            </div>
         </div>
      </div>
         ...
         <div className="divide-y divide-slate-50">
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
                    ...
                    <div className="space-y-6">
                       <div className="flex items-center gap-4">
                          ...
                          <div>
                             <p className="text-[10px] font-black uppercase text-indigo-600 mb-1">Customer</p>
                             <h4 className="text-sm font-black uppercase text-slate-900">{r.customerName || "Guest User"}</h4>
                          </div>
                       </div>
                       <div className="p-4 bg-slate-50 rounded-3xl flex items-center gap-4 border border-slate-100">
                          ...
                          <div className="min-w-0">
                             <p className="text-[9px] font-black uppercase text-slate-400 truncate">Product</p>
                             <h5 className="text-[11px] font-black uppercase text-slate-900 truncate">{r.productName}</h5>
                          </div>
                       </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-3 justify-center">
                       {!r.merchantReply ? (
                          <button className="w-full py-4 bg-white text-slate-900 text-slate-900 text-slate-900 border border-slate-100 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-indigo-600 transition-all shadow-xl">
                             <MessageSquare className="w-4 h-4" /> Add Reply
                          </button>
                       ) : (
                          <button className="w-full py-4 bg-slate-100 text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-3 cursor-not-allowed">
                             <CheckCircle2 className="w-4 h-4" /> Replied
                          </button>
                       )}
                       <button className="w-full py-4 border border-slate-100 text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-red-50 hover:text-red-600 hover:border-red-100 transition-all">
            ...
            {reviews.length === 0 && (
              <div className="py-32 text-center space-y-6">
                 ...
                 <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">No product reviews yet</p>
              </div>
            )}
         </div>
      </div>
